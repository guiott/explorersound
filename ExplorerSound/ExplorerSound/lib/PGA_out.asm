...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_out_Start
;  FUNCTION NAME: PGA_out_SetPower
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[PGA_out_GAIN_CR0]                           ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[PGA_out_GAIN_CR3],~EXGAIN                      ; Clear High Gain bit.
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_out_GAIN_CR0], A                           ; move complete value back to register
   or   reg[PGA_out_GAIN_CR3], EXGAIN                      ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
